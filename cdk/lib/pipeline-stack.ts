    ...
    const pipeline = new CodePipeline(this, 'ServerlessPipeline', {
      ...
      synth: new CodeBuildStep('SynthStep', {
        ...
        commands: [
          ...
          // export all env variables to .env file
          'ls -la cdk',
          'echo "export CDK_DEFAULT_ACCOUNT=$CDK_DEFAULT_ACCOUNT" >> .env',
          'echo "export CDK_DEFAULT_REGION=$CDK_DEFAULT_REGION" >> .env',
          "npx cdk synth --app 'npx ts-node --prefer-ts-exts cdk/bin/app.ts' --output cdk.out",
          // "npx cdk deploy --app 'npx ts-node --prefer-ts-exts cdk/bin/app.ts' --output cdk.out",
        ],
